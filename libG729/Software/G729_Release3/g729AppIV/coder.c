...
int main(int argc, char *argv[] )
{
  ...
  vadState_fx *vadSt_fx;
  Word16 vad_mode;
  Word16 SigFpw_fx[FFTSIZE16k];                                                	
  Word16 mssnr_fx;
  Word16 fft_exp_fx;
  Word16 vad_flag;
 ...
 *--------------------------------------------------------------------------*/
  ...
  */
  ...
  while( fread(new_speech, sizeof(Word16), L_FRAME, f_speech) == L_FRAME)
  {
    ...
    if (vad_enable)
    {
        preproc_fx(new_speech, SigFpw_fx, &fft_exp_fx,1, vadSt_fx);	
        vad_flag = vad_fx(new_speech, vadSt_fx, SigFpw_fx, &fft_exp_fx, vad_mode, 1, &mssnr_fx);
    }

    Pre_Process(new_speech, L_FRAME);

    Coder_ld8k(prm, syn, frame, vad_enable, vad_flag);
